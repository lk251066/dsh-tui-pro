...
The `1.8.1` source gives established conversations the full main-column height above the framed input. Persistent top chrome, the outer workbench frame, blank padding, and the repeated full-width input rule are removed; the welcome box remains limited to pristine sessions. A one-column inset and one dim sidebar divider preserve structure without enclosing the terminal. The bottom line reports the current step's rolling `token/s` estimate while output is streaming.
...
Conversation turns do not repeat `You` or `Assistant` labels. A shared two-column gutter reserves the first cell for meaning and aligns every continuation under the body: user messages use `›`, assistant replies use `•`, thinking uses `∴`, tool calls use their status glyph, and the first result row uses an indented `⎿`. Your messages retain a subtle full-width band while assistant Markdown stays bare. Plan review is the one deliberate bordered panel in the transcript, and delegated work is labelled `Subagent` with its prompt and result folded under the same tool-card controls. Shown thinking carries a dim `▎` quote bar. Code blocks use the same dim bar, and paired diff rows emphasize changed words in bold over the line color. A settled reply longer than `maxMessageLines` rendered lines folds to a head preview ending in a `… +M lines (click to expand)` row; a streaming reply always renders in full. `Ctrl+O` controls tool, Todo, diff, and context detail; `Ctrl+R` controls reasoning. The input frame stays dim and flags only plan mode and the danger permission preset.

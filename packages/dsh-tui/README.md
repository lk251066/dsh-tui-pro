...
A pristine conversation opens with a rounded welcome box: the block logo, the configured welcome line, the live model and directory, and a few suggested commands. The box sweeps in left-to-right on startup, the logo shimmers once, and any conversation traffic (or a titled session) swaps in the compact `dsh v{version} — {title}` header. The bottom status line contains cwd, branch, model, and context; the sidebar Status section contains agent state, plan mode, tokens, cache, and permission.
...
Conversation turns use `You` and `Assistant` headings stamped with their `HH:MM` time. Your own messages stand apart as a full-width band filled with a background a shade off the terminal's own (the one background fill in an otherwise foreground-only palette), the body opening with a `›` marker; shown thinking carries a dim `▎` quote bar, and a settled tool card keeps its status color only on the glyph while its title drops to dim. Code blocks carry the same dim `▎` left bar, and a paired diff row emphasizes its changed words in bold over the line color. A settled reply longer than `maxMessageLines` rendered lines folds to a head preview ending in a `… +M lines (click to expand)` row; a streaming reply always renders in full. Thinking, individual tool calls, grouped tool calls, diffs, and injected context show `▶` or `▼`; click their header to expand or collapse that block. `Ctrl+O` and `Ctrl+R` remain the global visibility controls. The input frame stays dim and flags only the two modes that change what a submission does: plan mode in accent, the danger permission preset in warning.

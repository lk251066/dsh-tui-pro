...
/** Keep fixture call sites readable while events carry UserMessage directly. */
function inboxItem(message: UserMessage, _placement: string): UserMessage {
  return message
}


class FakeTerminal implements Terminal {
  // Keep ordinary behavior tests wide enough for the production split view;
  // responsive tests set an explicit width when they exercise narrow panes.
  columns = 140
  rows = 32
...
describe('pi-tui chat lifecycle and transcript', () => {
  ...
  it('badges queued steering on the prompt context and clears it as each drains', async () => {
    ...
    result.terminal.output = ''
    agentEvents(result.ctx, other).emit('agent/inbox/inserted', { message: inboxItem(freezeMessage({
      id: MessageId('stub'),
      role: 'user',
    ...
    // the badge alone.
    agentEvents(result.ctx, other).emit('agent/inbox/claimed', {
      message: inboxItem(discarded[0]!, 'steering'),
      turn: 1,
    })
    agentEvents(result.ctx, result.agent).emit('agent/inbox/claimed', { message: inboxItem(freezeMessage({
    ...
    for (const message of discarded) {
      agentEvents(result.ctx, other).emit('agent/inbox/discarded', {
  ...
  it('releases the live compaction timer and progress bit on dispose', async () => {
    ...
    try {
      ...
      result.session.append('compaction/start', { turn: null })
      expect(intervalSpy).toHaveBeenCalledOnce()
      const compactionTimer = intervalSpy.mock.results[0]?.value

      await dispose(result)
      didDispose = true
      expect(clearIntervalSpy).toHaveBeenCalledWith(compactionTimer)
      expect(result.terminal.progress.at(-1)).toBe(false)
    } finally {
      if (result !== undefined && !didDispose) await dispose(result)
      intervalSpy.mockRestore()
  ...
  it('tracks steering drains without a running status line', async () => {
    ...
    agentEvents(result.ctx, result.agent).emit('agent/inbox/inserted', { message: inboxItem(freezeMessage({
      id: MessageId('stub'),
      role: 'user',
  ...
  it('injects referenced context before steering while the agent is running', async () => {
    const result = await setup({
      status: 'running',
      omitInitialLifecycle: true,
      async configureContext(ctx) {
  ...
  it('injects prepared reference context before each prompt delivery', async () => {
    const result = await setup({
      async configureContext(ctx) {
        ctx.provide('tools', { get: () => undefined } as never)
        await ctx.plugin(TestSessionQueryService)
        await ctx.plugin(SessionReferenceResolver)
        const source = ctx.sessions.create(SessionId('prepared-source'), {
          meta: { cwd: process.cwd(), createdAt: 1 },
        })
        appendUser(source, 'source background')
      },
    ...
    const sendReference = async (): Promise<void> => {
      result.terminal.send('@prepared-source')
      await vi.waitFor(() => {
        expect(result.terminal.output).toContain('Session · prepared-source')
      })
      result.terminal.send('\t')
      await tick()
      result.terminal.send('\r')
    ...
    expect(result.agent.injected).toHaveLength(2)
    expect(result.agent.injectedOptions.map(message => message.source)).toEqual([
      expect.objectContaining({
        kind: 'session-reference',
        references: [expect.objectContaining({ sessionId: 'prepared-source' })],
      }),
      expect.objectContaining({
        kind: 'session-reference',
        references: [expect.objectContaining({ sessionId: 'prepared-source' })],
  ...
  it('cancels before /quit while running and handles agent errors/disposal', async () => {
    ...
    expect(events.terminal.output).toContain('Turn ended: disposed')
...
describe('terminal mounting', () => {
  ...
  it('renders an uncoercible startup failure without escaping the display boundary', async () => {
    ...
    mountTui(ctx, { sessionId: 'main-session', theme: { color: false } }, { terminal, exit })
    ctx.emit('agent-loop/config-start-failed', { sessionId: SessionId('main-session'), error: {

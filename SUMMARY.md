...
The `1.8.1` source removes the persistent top header from established conversations. The input frame supplies the lower boundary without a repeated full-width rule, so the transcript receives every available row above it; the welcome box remains limited to pristine sessions. Subtle user bands and assistant Markdown distinguish speakers without separate `You` or `Assistant` heading rows. Assistant replies begin one row after user messages, the input prompt has no `dsh` prefix, and the bottom line shows the current step's rolling output rate only while streaming.
